...
const Mutations = {
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    // not using query user, because it requires unique inputs
    // email and id - so it's users which also has many more methods
    // for querying, destructure the user from the returned array
  ...
  async updatePermissions(parent, args, ctx, info) {
    ...
    return ctx.db.mutation.updateUser(
      {
        data: {
          // permissions have to be set bc they're enum
          permissions: {
            set: args.permissions,
          },
        },
        where: {
          id: args.userId, // we may be updating someone else
        },
      },
      info
    );
